    ...
    class GhostResponseHandler
    {
        ...
        public void InvokeEvent(int event_id, long handler_id, byte[][] event_params)
        {
            ...
            if (fieldValue is GhostEventHandler fieldValueDelegate)
            {
                ...
                {
                    PinionCore.Utility.Log.Instance.WriteInfo(string.Format("Call event error in {0}:{1}. {2}", type.FullName, info.Name, tie.InnerException.ToString()));
                    throw tie;
                }
                catch (Exception e)
                {
                    PinionCore.Utility.Log.Instance.WriteInfo(string.Format("Call event error in {0}:{1}.", type.FullName, info.Name));
                    throw e;
                }
            }
