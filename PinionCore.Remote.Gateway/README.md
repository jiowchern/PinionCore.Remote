...
classDiagram
    ...
    class SessionHub {
        ...
        -Dictionary~VersionKey, SessionCoordinator~ _Coordinators
        -Entry _Entry
    ...
    class Entry {
        -ISessionMembershipProvider _provider
        -Dictionary~ISessionBinder, User~ _Users
        +OnSessionOpened(ISessionBinder)
        +OnSessionClosed(ISessionBinder)
    }

    class Registry {
        +IAgent Agent
        +IListenable Listener
        +Registry(IProtocol protocol, uint group)
        +Dispose()
    }
...
// 建立 Router（傳入路由策略，預設可用 RoundRobinSelector）
...
// 建立遊戲服務（IEntry 繼承 ISessionObserver，連線的開啟/關閉由 ISessionBinder 承接）
...
// 建立 Registry (指定協議與 Group ID = 1；Router 以協議版本做隔離)
var registry = new Registry(protocol, 1);

// 綁定遊戲服務到 Listener
registry.Listener.StreamableEnterEvent += gameService.Join;
registry.Listener.StreamableLeaveEvent += gameService.Leave;

// 連接到 Router：對 Router 的 Registry 端點開一條 Standalone 連線，
// 並把串流交給 registry.Agent（與測試中的 ConnectAgent 輔助方法相同）
var endpoint = new PinionCore.Remote.Standalone.ListeningEndpoint();
var (handle, errors) = await router.Registry.ListenAsync(endpoint);
PinionCore.Remote.Client.IConnectingEndpoint connectable = endpoint;
registry.Agent.Enable(await connectable.ConnectAsync());

// 之後需在迴圈中定期呼叫 registry.Agent.HandleMessages() / HandlePackets()
```
...
// 建立 Agent (需要提供遊戲協議；成員為明確介面實作，請以 IAgent 型別持有)
PinionCore.Remote.Ghost.IAgent agent = new Agent(new AgentPool(gameProtocol));

// 連接到 Router 的 Session 端點（與 Registry 相同的連線方式）
var endpoint = new PinionCore.Remote.Standalone.ListeningEndpoint();
var (handle, errors) = await router.Session.ListenAsync(endpoint);
PinionCore.Remote.Client.IConnectingEndpoint connectable = endpoint;
agent.Enable(await connectable.ConnectAsync());
...
public class Registry : IDisposable
{
    ...
    // protocol - 遊戲協議（Router 以其 VersionCode 做版本隔離）
...
```csharp
// 服務類型 A (Group 1)
var registryA = new Registry(protocol, 1);

// 服務類型 B (Group 2)
var registryB = new Registry(protocol, 2);
...
var registryV1 = new Registry(oldProtocol, group: 1);

// 新版本服務
var newProtocol = NewProtocolCreator.Create(); // VersionCode = [2, 0, 0]
var registryV2 = new Registry(newProtocol, group: 1);
...
3. **版本相容性檢查**：
   ...
   var registry = new Registry(protocol, group: 1);
...
1. **訊息迴圈的重要性**: Registry 和 Agent 都需要定期呼叫 `HandleMessages()` 與 `HandlePackets()`（可參考測試中的 `AgentWorker` 輔助類，以背景執行緒代為輪詢）
